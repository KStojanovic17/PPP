...
type
  ...
  TAccountFrame = class(TMainFrame)
...
procedure TAccountFrame.CaptionLblApplyStyleLookup(Sender: TObject);
begin
  inherited;
...
procedure TAccountFrame.DoOnDidFinishTaking(Image: TBitmap);
begin
...
procedure TAccountFrame.ProfileSvgClick(Sender: TObject);
begin
...
procedure TAccountFrame.SignOutBtnClick(Sender: TObject);
begin
...
procedure TAccountFrame.SignOutHitBoxClick(Sender: TObject);
begin
  GetMainForm.ShowActivity(sActivitySignInName, true);
end;
...
initialization
  ...
  RegisterClass(TAccountFrame);
finalization
  // Unregister profile frame class
  UnRegisterClass(TAccountFrame);
